...
module cover(print_orientation=1,baseFrameMod=0,prof=30,screw=8)
{
    if(baseFrameMod==1){
        minko=3;
        
        translate([-40+minko,-(cover_size[x]/2),-prof+minko+11.5])
        difference(){
            minkowski(){
                
                rotate([90,0,0])
                //translate([0,0,-2.5])
                cube([80-(minko*2),prof-(minko*2),5]);
                rotate([90,0,0])
                cylinder(r=minko,h=0.01,center=true);
            }
             translate([40-minko,0,(prof/2)-minko])
            for(f=[0:1]){
                rotate(f*180)
                translate([30,0,0])
                rotate([90,0,0])
                cylinder(d=screw+0.3,h=100,center=true);
            }
        }
    }
	...
	difference()
	{
		...
		translate(cover_center)
            minkowski()
            {
                ...
                cylinder(h=1, r=2, $fn=48, center=true);
            }
		...
		*translate(cover_center)
		...
		cylinder(h=cover_z1-cover_z0+.1, r=planet_carrier_radius+1, $fn=160, center=true);

		translate([0, 0, (cover_z1+cover_z2)/2-.05])
		cylinder(h=cover_z2-cover_z1+.1, r=(output_bearing[bearing_body_diameter]+output_bearing_clearance[bearing_body_diameter])/2, $fn=120, center=true);

		translate(cover_center)
		cylinder(h=cover_size[z]+.1, r=(output_bearing[bearing_rod_diameter]+output_bearing_clearance[bearing_rod_diameter])/2, $fn=80, center=true);

		for(i=[0:3]) rotate([0, 0, i*90])
		{
			translate([motor_mount_hole_spacing/2, motor_mount_hole_spacing/2, (cover_bolt_z0+cover_bolt_z1)/2-layer_height])
			cylinder(h=(cover_bolt_z1-cover_bolt_z0)+0.75, r=m3_diameter/2, $fn=48, center=true);

			
			translate([motor_mount_hole_spacing/2, motor_mount_hole_spacing/2, (cover_bolt_z1+cover_bolt_z2)/2+.05])
			cylinder(h=cover_bolt_z2-cover_bolt_z1+.1, r=m3_bolt_head_diameter/2, $fn=48, center=true);
		}
	}
}
...
cover(print_orientation=0,baseFrameMod=1,prof=20,screw=5);
